...
<?php endif; ?>
        ...
        <div class="flex-1 flex flex-col ml-64 main-content">
            <div class="bg-gray-50 content-wrapper" x-data="brandingManager()">
                ...
                <?php if($user->isSuperAdminNode() && count($availableNodes) > 0): ?>
...
function brandingManager() {
    return {
        inheritFromParent: <?php echo json_encode($currentBranding['inherit_from_parent'] ?? false, 15, 512) ?>,
        colors: {
            primary_color: <?php echo json_encode($currentBranding['primary_color'] ?? '#3B82F6', 15, 512) ?>,
            secondary_color: <?php echo json_encode($currentBranding['secondary_color'] ?? '#6B7280', 15, 512) ?>,
            accent_color: <?php echo json_encode($currentBranding['accent_color'] ?? '#10B981', 15, 512) ?>,
            text_color: <?php echo json_encode($currentBranding['text_color'] ?? '#1F2937', 15, 512) ?>,
            background_color: <?php echo json_encode($currentBranding['background_color'] ?? '#FFFFFF', 15, 512) ?>
        },
        fontFamily: <?php echo json_encode($currentBranding['font_family'] ?? 'Inter', 15, 512) ?>,
        customCss: <?php echo json_encode($currentBranding['custom_css'] ?? '', 15, 512) ?>,
        logoPreview: <?php echo json_encode(($currentBranding['logo_url'] ?? null) ? asset('storage/' . $currentBranding['logo_url']) : null, 15, 512) ?>,
        logoSmallPreview: <?php echo json_encode(($currentBranding['logo_small_url'] ?? null) ? asset('storage/' . $currentBranding['logo_small_url']) : null, 15, 512) ?>,
        faviconPreview: <?php echo json_encode(($currentBranding['favicon_url'] ?? null) ? asset('storage/' . $currentBranding['favicon_url']) : null, 15, 512) ?>,
        saving: false,
        uploadedFiles: {},
