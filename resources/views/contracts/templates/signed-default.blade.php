...
<head>
    ...
    <title>Contrato Assinado - {{ $contract->id ?? 'N/A' }}</title>
    <style>
        body {
...
<body>
    ...
    <div class="contract-info">
        <table>
            <tr>
                ...
                <td>#{{ str_pad($contract->id ?? 0, 6, '0', STR_PAD_LEFT) }}</td>
            </tr>
            <tr>
                <td>Data de Criação:</td>
                <td>{{ isset($contract) ? $contract->created_at->format('d/m/Y H:i') : date('d/m/Y H:i') }}</td>
            </tr>
            <tr>
                <td>Status:</td>
    ...
    <div class="content">
        ...
        <div class="clause">
            ...
            <p>
                <strong>{{ $licenciado->name ?? 'Nome do Licenciado' }}</strong>, 
                {{ strlen($licenciado->cnpj_cpf ?? '') == 14 ? 'pessoa jurídica' : 'pessoa física' }}, 
                portador do documento {{ $licenciado->cnpj_cpf ?? 'N/A' }}, 
                com endereço {{ $licenciado->endereco_completo ?? 'não informado' }}, 
                CEP {{ $licenciado->cep ?? 'não informado' }}, 
                e-mail {{ $licenciado->email ?? 'não informado' }}.
            </p>
        </div>
    ...
    <div class="signature-section">
        ...
        <div class="signature-info">
            <div class="row">
                ...
                <div class="cell">{{ $signature_info['signer_name'] ?? 'N/A' }}</div>
            </div>
            <div class="row">
                <div class="cell">Documento:</div>
                <div class="cell">{{ $signature_info['signer_document'] ?? 'N/A' }}</div>
            </div>
            <div class="row">
                <div class="cell">Data/Hora da Assinatura:</div>
                <div class="cell">{{ $signature_info['signed_at'] ?? 'N/A' }}</div>
            </div>
            <div class="row">
                <div class="cell">IP de Origem:</div>
                <div class="cell">{{ $signature_info['signer_ip'] ?? 'N/A' }}</div>
            </div>
            <div class="row">
        ...
        <div class="signature-image">
            ...
            <img src="{{ $signature_image_path }}" alt="Assinatura Digital" />
        </div>
        @endif
