...
@section('title', 'Pendentes de Aprovação')
@section('subtitle', 'Solicitações aguardando sua aprovação')
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                <div class="text-sm text-gray-600 bg-orange-50 px-3 py-2 rounded-lg border border-orange-200">
                    ...
                    <strong>{{ $agendas->count() }}</strong> solicitação(ões) pendente(s)
                </div>
                ...
                <div class="p-6">
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            ...
                            <div class="flex items-center gap-3 mb-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600">
                                        Solicitado por: <strong>{{ $agenda->solicitante->name ?? 'N/A' }}</strong>
                                    </p>
                                </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                                <div class="flex items-center text-sm text-gray-600">
                                    ...
                                    <span>{{ \Carbon\Carbon::parse($agenda->data_inicio)->format('d/m/Y') }}</span>
                                </div>
                                <div class="flex items-center text-sm text-gray-600">
                                    <i class="fas fa-clock mr-3 text-purple-500 w-4"></i>
                                    <span>{{ \Carbon\Carbon::parse($agenda->data_inicio)->format('H:i') }} - 
                                          {{ \Carbon\Carbon::parse($agenda->data_fim)->format('H:i') }}</span>
                                </div>
                                <div class="flex items-center text-sm text-gray-600">
                                    <i class="fas fa-map-marker-alt mr-3 text-purple-500 w-4"></i>
                                    <span>{{ ucfirst($agenda->tipo_reuniao) }}</span>
                                </div>
                                @if($agenda->participantes && count($agenda->participantes) > 0)
                                    <div class="flex items-center text-sm text-gray-600">
                                        <i class="fas fa-users mr-3 text-purple-500 w-4"></i>
                                        <span>{{ count($agenda->participantes) }} participante(s)</span>
                                    </div>
                                @endif
                                <div class="flex items-center text-sm text-gray-600">
                            ...
                            @if($agenda->descricao)
                                <div class="mb-4">
                                    ...
                                    <p class="text-sm text-gray-600 bg-gray-50 p-3 rounded-lg">{{ $agenda->descricao }}</p>
                                </div>
                            @endif
                                ...
                                <div class="mb-4">
                                    ...
                                    <a href="{{ $agenda->google_meet_link ?? $agenda->meet_link }}" 
                                       ...
                                       class="inline-flex items-center text-sm text-blue-600 hover:text-blue-800 transition-colors">
                                        ...
                                        {{ $agenda->google_meet_link ?? $agenda->meet_link }}
                        ...
                        <div class="flex flex-col gap-3 ml-6">
                            ...
                            <button onclick="verDetalhes({{ $agenda->id }})" 
